...
RAW = ROOT / "raw"
RAW_IPAD = ROOT / "raw-ipad-13"
FINAL = ROOT / "final"
FINAL_65 = ROOT / "final-6.5"
FINAL_IPAD = ROOT / "final-ipad-13"
CANVAS_SIZE = (1320, 2868)
IPHONE_65_SIZE = (1284, 2778)
IPAD_13_SIZE = (2064, 2752)
...
def create_contact_sheet(outputs: list[Path], destination: Path) -> None:
    columns = 3
    thumb_width = 330
    first = Image.open(outputs[0])
    thumb_height = round(first.height * thumb_width / first.width)
    rows = (len(outputs) + columns - 1) // columns
    sheet = Image.new("RGB", (thumb_width * columns, thumb_height * rows), (10, 10, 14))
...
def export_iphone_65(source: Image.Image) -> Image.Image:
    target_width, target_height = IPHONE_65_SIZE
    ...
    return resized.crop((0, crop_top, target_width, crop_top + target_height)).convert("RGB")


def export_ipad(source_name: str) -> Image.Image:
    source = Image.open(RAW_IPAD / source_name)
    if source.size != IPAD_13_SIZE:
        raise ValueError(
            f"Expected a {IPAD_13_SIZE[0]}x{IPAD_13_SIZE[1]} iPad capture for {source_name}, "
...
def main() -> None:
    ...
    FINAL_IPAD.mkdir(parents=True, exist_ok=True)
    outputs: list[Path] = []
    outputs_65: list[Path] = []
    outputs_ipad: list[Path] = []
    manifest = {
        "app": "Pace & Plates",
        "version": "2.5",
        "build": "4",
        ...
        "sets": {
            "iphone_6_9": {"directory": "final", "dimensions": list(CANVAS_SIZE)},
            "iphone_6_5": {"directory": "final-6.5", "dimensions": list(IPHONE_65_SIZE)},
            "ipad_13": {"directory": "final-ipad-13", "dimensions": list(IPAD_13_SIZE)},
        },
        "screenshots": [],
    }
    for item in SCREENS:
        output = FINAL / str(item["output"])
        rendered = render_screen(item)
        rendered.save(output, format="PNG", optimize=True)
        outputs.append(output)

        output_65 = FINAL_65 / str(item["output"])
        export_iphone_65(rendered).save(output_65, format="PNG", optimize=True)
        outputs_65.append(output_65)

        output_ipad = FINAL_IPAD / str(item["output"])
        export_ipad(str(item["source"])).save(output_ipad, format="PNG", optimize=True)
        outputs_ipad.append(output_ipad)

        manifest["screenshots"].append(
            {
                "file": output.name,
                "source": f"raw/{item['source']}",
                "ipad_source": f"raw-ipad-13/{item['source']}",
                "headline": item["headline"],
                "subhead": item["subhead"],
            }
        )
    (ROOT / "manifest.json").write_text(json.dumps(manifest, indent=2) + "\n", encoding="utf-8")
    create_contact_sheet(outputs, ROOT / "contact-sheet.jpg")
    create_contact_sheet(outputs_ipad, ROOT / "contact-sheet-ipad.jpg")
